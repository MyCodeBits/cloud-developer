...
(async () => {
  ...
  app.get( "/cars/:id", ( req: Request, res: Response ) => {
    ...
    // Respond not found, if we donot have the id
    if (car && car.length === 0) {
      return res.status(404).send(`Car is not found with the passed-in id : ` + id)
    }
  ...
  app.post( "/cars/", ( req: Request, res: Response ) => {
    ...
    if (!id || !type || !model || !cost || !make) {
      // respond with an error if not
      return res.status(400)
        .send(`All the required fields :  'make', 'type', 'model', 'cost', 'id' are required`);
    }
    ...
    // add this care to our local variable
    cars.push(new_car);
